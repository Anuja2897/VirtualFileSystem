...
void man(char *name)
{
	...
	else if(stricmp(name,"read")==0)
	{
		...
		printf("Usage : read file_name ni_of_bytes_to_read \n");
	}
	...
	{
		printf("Description : used to closed the open file \n");	
...
void DisplayHelp()
{
	...
	printf("fstate : to display information of fileusing the descriptor \n");
...
int rm_File(char * name)
{
	...
	if(UFDTArr[fd].ptrfiletable->ptrinode->LinkCount == 0)
	{
		...
		if(stricmp(UFDTArr[fd].ptrfiletable->ptrinode->FileName,name)==0)
		memset(UFDTArr[fd].ptrfiletable->ptrinode->FileName,0,sizeof(UFDTArr[fd].ptrfiletable->ptrinode->FileName));
...
void ls_file()
{
	...
	printf("\n File Name \t Inode number \t FileSize \t LinkCount \n");
	printf("---------------------------------------------------------\n");
...
int fstat_file(int fd)
{
	...
	{
		printf("file permissiom : Read only \n");
	}
...
int stat_file(char *name)
{
	...
	printf("\n=--------staticstical informaton about file-------------\n");
	printf("FileNname : %s\n",temp->FileName);
...
int main()
{
	...
	char command[4][80],arr[1024],str[80];
	...
	while(1)
	{
		...
		case 1:
			...
			else if(stricmp(command[0],"exit")==0)
			{
				...
				printf("Do you really want to terminate our project ?")
				break;
			}
		...
		case 2:
			...
			if(stricmp(command[0],"stat")==0)
			{
				...
				if(ret == -2)
				{
					printf("ERROR : there is no such a files \n");
				}
			}
			else if(stricmp(command[0],"fstat")==0)
			{
				...
				if(ret == -1)
				{
					printf("ERROR : incorrect paramertes \n");
				}
				if(ret == -2)
				{
					printf("ERROR : there is no such a files \n");
			...
			else if(stricmp(command[0],"close")==0)
			{
				...
				else if(stricmp(command[0],"write")==0)
				{
					...
					if(ret == 0)
					{
						printf("Error : Incorrect Paramerte \n");
						continue;
					}
					...
					if(ret == -2)
					{
						printf("ERROR : there is no sufficent memory to write \n");
					}
					if(ret == -3)
					{
						printf("EROR : it is not regular file \n");
					}
			...
			case 3:
				...
				if(stricmp(command[0],"create")==0)
				{
					...
					if(ret >= 0)
					{
						printf("file is succesfully create with file discriptor : %d\n",ret);
					}
					if(ret == -1)
					{
						printf("ERROR : Incorrect parameters \n");
